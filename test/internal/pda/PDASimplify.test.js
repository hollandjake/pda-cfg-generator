...
test('singleAcceptState', () => {
    ...
    expect(Transition.sort(transitions)).toEqual(Transition.sort([
        ...
        new Transition(State.p(1), State.p(4)),
        new Transition(State.p(2), State.p(4)),
        new Transition(State.p(3), State.p(4)),
    ]));

    expect(newAcceptState).toEqual(State.p(4))
})
...
test('Empty Stack', () => {
    ...
    let transitions1 = PDASimplify.emptyStack(pda.transitions, State.p(2,false), pda);
...
test('transitionSplitting', () => {
    ...
    expect(Transition.sort(PDASimplify.transitionSplitting(pda.transitions, pda))).toEqual(Transition.sort([
        new Transition(State.p0,State.p(1,true), InputSymbol.of('b'), StackSymbol.EPSILON, StackSymbol.of('D')),
        new Transition(State.p0,State.p(2),InputSymbol.EPSILON, StackSymbol.EPSILON, StackSymbol.of('E')),
        new Transition(State.p(2),State.p(1,true),InputSymbol.EPSILON, StackSymbol.of('E'),StackSymbol.EPSILON),
        new Transition(State.p0,State.p(3),InputSymbol.of('a'), StackSymbol.of('B'), StackSymbol.EPSILON),
        new Transition(State.p(3),State.p(1,true),InputSymbol.EPSILON, StackSymbol.EPSILON,StackSymbol.of('C')),
    ]));
})
...
test('simplify', () => {
    ...
    expect(actual).toEqual(PDA.fromTransitions([
        ...
        new Transition(State.p0, State.p(2), InputSymbol.EPSILON, StackSymbol.EPSILON,StackSymbol.of("B")),
        new Transition(State.p(2), State.p(1), InputSymbol.EPSILON, StackSymbol.of('B'),StackSymbol.EPSILON),
