...
test('step4 with p\u209B start state', () => {
    let pda = PDA.fromTransitions([
        new Transition(State.start, State.p(1), InputSymbol.of('a'), StackSymbol.EPSILON, StackSymbol.of('T')),
        new Transition(State.p(2), State.p(3, true), InputSymbol.of('b'), StackSymbol.of('T'), StackSymbol.EPSILON),
    ], State.start);

    let [rules, startVariable] = PDAConvert.step4([], pda);

    expect(rules).toEqual([new Rule(Variable.S, [Variable.A('psp3')])])
    expect(startVariable).toEqual(Variable.S);
})
